...
Exploratory data analysis and preliminary statistical analysis on the Spotify Music dataset from the TidyTuesday project are conducted in the report in hopes of addressing three overarching questions exploring the relationships between different variables in the Spotify Music dataset. We make use of 9 different plots for our visualization: histograms for quantitative variables, scree plot, principle component analysis (PCA), ridgeline plot, stacked bar plot, marginal distribution plot, scatterplots with contour lines, correlogram, and scatterplots with linear regression lines. We conduct chi-squared test to test whether the proportion of genre appearing in top 100 songs are equal. We could not find a clear relationship between track popularity and other quantitative variables using all the songs in the dataset. We found that the marginal distribution of different genre of top 100 songs are different both from the marginal distribution graph, and from the results of our chi-squared test. We also found that most songs have danceability in between 0.5 to 0.75 regardless of genre and speechiness from 0 to 0.25. It seemed like the songs with higher popularity usually were tagged with more than one genre. Finally, we found that there were some pairs of variables with high correlations. A more complex regression analysis could be performed on the dataset to gain a further understanding of the characteristics of the songs on the track popularity. Some variables might need to be transformed. Also, a recommendation system would be a good next-step.
...
```{r pop-pca, echo=FALSE, fig.cap="PCA plot for PC1 and PC2", fig.height=4, fig.width=5}
...
pop_pca <- pop_pca_df %>%
  ...
  labs(x = "PC 1", y = "PC 2", color = "Track Popularity",
       title = "PCA between PC1 and PC2 showing no relationship")

plot(pop_pca)
```

```{r genre-popularity, fig.height=4, fig.width=5, echo=FALSE, fig.cap="Ridgeline plot of track popularity by genre"}
# Graph code
...
```{r spotify-100, echo=FALSE, fig.height=4, fig.width=5}
...
```{r genre-100, echo=FALSE, fig.cap="Stacked bar plot of genre conditioned on subgenre", fig.height=4.25, fig.width=6.5}
bar_100 <- spotify_100 %>% 
  ggplot(aes(x = playlist_genre, fill = playlist_subgenre)) +
  geom_bar() +
  labs(x = "Genre", y = "Count", fill = "Subgenre",
       title = "Pop, R&B, Latin have the highest count")

plot(bar_100)
```

```{r marginal-dist-ci, echo=FALSE, fig.cap="Marginal distribution of genre with CI", fig.height=4, fig.width=5.5}
...
spo_con <- spotify %>%
  ...
  labs(
    ...
    color = "Genre",
    title = "All Genre",
    subtitle = "Separated by color"
  )

rnb_con <- spotify_100[spotify_100$playlist_genre == "r&b",] %>%
  ggplot(aes(x = danceability, y = speechiness)) +
...
pop_con <- spotify_100[spotify_100$playlist_genre == "pop",] %>%
  ggplot(aes(x = danceability, y = speechiness)) +
  geom_point(alpha = 0.25) +
  geom_density2d() +
  coord_fixed() +
...
latin_con <- spotify_100[spotify_100$playlist_genre == "latin",] %>%
  ggplot(aes(x = danceability, y = speechiness)) +
  geom_point(alpha = 0.25) +
  geom_density2d() +
  coord_fixed() +
...
grid.arrange(spo_con, pop_con, rnb_con, latin_con, ncol=2, nrow=2)
```

``` {r multi-genre, echo=FALSE, fig.cap="Scatterplot showing number of genre tagged and the popularity of the song", warning=FALSE, message=FALSE, fig.height=4, fig.width=5.5}
...
```{r corr, fig.height=4, fig.width=4, fig.cap="Correlation matrix", echo=FALSE}
...
sca_el <- spotify %>%
  ...
  labs(
    x = "Energy",
    y = "Loudness",
    title = "Scatterplot of loudness and energy with linear trend for 99% CI",
...
sca_ae <- spotify %>%
  ...
  labs(
    x = "Energy",
    y = "Acousticness",
    title = "Scatterplot of acousticness and energy with linear trend for 99% CI",
    subtitle = "Decreasing linear relationship between acousticness and energy"
  )

grid.arrange(sca_el, sca_ae, nrow=2)
```
...
The scree plot (which is not included) and principle component analysis will help us determine what quantitative variables are related to track popularity, which will help us answer our first question. The scree plot shows that the first three dimensions explain about 48.2% of total variation in the data.

The PCA plot (Figure 1) does not show a clear relationship between track popularity and the two dimensions.

Ridgeline plot (Figure 2) of track popularity by genre will show us whether different genres have different distribution of track popularity. We see that all genres contain a mode of low track popularity, and a second mode with higher track popularity. 

In order to answer our second question, stacked bar plot (Figure 3) displaying which genres have the highest counts in top 100 songs conditioned by subgenre was plotted. It looks like Latin, R&B, and Pop are the three most popular genres in the top 100 songs.

For top 100 songs, we will plot a bar graph (Figure 4) displaying marginal distribution of genre with confidence intervals. We will use Bonferroni correction to calculate the confidence intervals. We will also perform a chi-squared test to see if the proportions of genre are equal for top 100 songs.
...
From the variable description, it seems like `danceability` and `speechiness` are the two factors that differ between the genres. We will plot a scatterplot with contour lines for `danceability` and `speechiness` (Figure 5). There is one mode in the graph where most of the data points are located, which suggests that most songs have `danceability` in between 0.5 to 0.75 regardless of genre and `speechiness` from 0 to 0.25. There are some rap songs with high `speechiness` from 0.5 to close to 1.

Looking at the scatterplot with contour lines (Figure 5) for three most popular genres for top 100 songs (R&B, Pop, Latin), we see that most popular songs have `danceability` between 0.5 to 0.9 and low `speechiness`.

While examining the data, we found that some songs get tagged into multiple genres so there are duplicates in the dataset. The relationship between how many genre a song has and it’s popularity was graphed using a scatterplot (Figure 6). The scatterplot shows that track popularity tends to increase as the song gets tagged into multiple genre.
...
Correlogram (Figure 7) will show us which variables are correlated with each other. It looks like `energy` and `loudness` have strongest positive correlation and and `energy` and `acousticness` have the strongest negative correlation.

Finally, we will look at the scatterplots (Figure 8) with linear regression lines for `energy` and `loudness` and `energy` and `acousticness` because they are strongly correlated variables. It looks like the data points follow a linear trend for both pairs.

The link to our Tempo-Popularity ShinyAPP is [https://menciuschin.shinyapps.io/dataviz-finalshiny/](https://menciuschin.shinyapps.io/dataviz-finalshiny/)
...
PCA analysis was performed on the quantitative variables (song characteristics), even though the scree plot indicated that up to PC3 is good enough for explaining the dimensions, the scatterplot embedded `track_popularity` is too chaotic to observe any signs or signals between the variables. Logically, the popularity wouldn't be explained simply by a few predictors individually. Thus for the later questions, the analysis will be performed on a smaller scale such as by genre or for the top 100 popular songs.

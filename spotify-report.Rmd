...
From the histogram, it looks like `key` and `mode` are actually categorical variables. The variables `danceability`, `energy`, and `loudness` looks left-skewed, and `liveness`, `duration_ms`, `speechiness`, `acousticness`, and `instrumentalness` looks right-skewed.
...
From the variable description, it seems like danceability and speechiness are the two factors that differ between the genres. We will plot a scatterplot with contour lines for danceability and speechiness.
...
``` {r multi-genre, echo=FALSE, fig.cap="Scatterplot showing number of genre tagged and the popularity of the song"}
...
```{r corr-scatterplots, echo=FALSE, fig.cap="Scatterplots with linear regression for highly correlated variables", warning=FALSE}
spotify %>%
...
Based on both the graph output and the Chisq-test performed, the marginal distribution of different genre of top 100 songs are different. From the perspective of the bar graphs, as the 95\% CI indicated, since some confidence interval of genre didn't overlap, it is highly confident that the marginal distributions are also different; or from the p-value provided by Chi-squared test, which is much smaller than our $\alpha = 0.05$, we will reject our null and say the marginal distributions are different.
